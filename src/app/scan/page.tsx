...
export default function ScanPage() {
  ...
  const startScanner =
    useCallback(async () => {
      /*
       * 今日スタンプを獲得済みかどうかに関係なく、
       * QRコードのカメラは毎回起動する。
       *
       * スタンプを増やせるかどうかは、
       * 読み取り成功後のaddStamp()で判定する。
       */
          ...
          (
                ...
                if (
                  ...
                  setResult({
                    ...
                    description:
                      `研究室への入室を記録しました。` +
                      `今月の来室スタンプは` +
                      `${stampResult.stampData.stampCount}個目です。`,
                  });
                } else {
                  setResult({
                    status: "success",
            ...
            setResult({
              ...
              title:
                "読み取りに失敗しました",
              description:
                "このQRコードはチェックイン用ではありません。再読み込みボタンを押して、正しいQRコードを読み取ってください。",
            });
